/* ============================================================
   TACTICAL NOIR V2.0 — INTERACTIVE ENGINE
   Three.js Particle Field + HUD Systems + Micro-interactions
   V2: Timeline declassification + decryption text animation
   ============================================================ */
...
(function () {
  ...
  function initCustomCursor() {
    ...
    const interactives = document.querySelectorAll('a, button, .glass-card, .tech-tag, .competence-item, .intel-card, .declassify-btn');
  ...
  // ── V2: TIMELINE DECLASSIFICATION ──
  function initTimelineExpand() {
    const buttons = document.querySelectorAll('.declassify-btn');
    buttons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const ariaTarget = btn.getAttribute('aria-expanded') === 'true';

        // Toggle
        entry.classList.toggle('expanded');
        btn.setAttribute('aria-expanded', String(!ariaTarget));
        playClickSound();

        // On expand: run decryption animation on text elements
        if (!isExpanded) {
          const decryptEls = entry.querySelectorAll('.decrypt-text');
          decryptEls.forEach((el, i) => {
            setTimeout(() => decryptText(el), i * 200);
          });
          playDecryptSound();
        }
      });
  ...
  // ── V2: DECRYPTION TEXT SCRAMBLE ANIMATION ──
  function decryptText(element) {
    ...
    const interval = setInterval(() => {
      const revealed = finalText.substring(0, iteration);
      let scrambled = '';
      ...
      // Reveal ~3-5 characters per frame for reasonable speed
      iteration += Math.ceil(length / 40);
  ...
  function playDecryptSound() {
    ...
    try {
      ...
      osc.frequency.linearRampToValueAtTime(1200, state.audioCtx.currentTime + 0.15);
      osc.frequency.linearRampToValueAtTime(400, state.audioCtx.currentTime + 0.3);
